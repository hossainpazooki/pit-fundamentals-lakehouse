...
class PipelineSpec extends AnyFunSuite with SparkTestSupport {
  ...
  test("segment/coregistrant rows are scoped out of silver and gold, and counted in metrics") {
    withTempDir { root =>
      val cfg = AppConfig(
        SparkCfg("t", Some("local[2]")),
        PathsCfg(
      ...
      val m = Pipeline.runFromFrames(
        ...
        cfg,
        num = Fixtures.numDf(spark, filings),
        sub = Fixtures.subDf(spark, filings.take(1)), // one filing; the three rows share adsh
        batchId = "b-scope",
        ingestTs = Timestamp.valueOf("2023-05-02 00:00:00")
      )

      assert(m.gateOutcome == "Pass")
      assert(m.scopedOut == 2L, s"scoped_out was ${m.scopedOut}")
      val gold = spark.read.format("delta").load(cfg.paths.goldRoot)
      assert(gold.count() == 1L, "gold must hold only the entity-level fact")
    }
  }
